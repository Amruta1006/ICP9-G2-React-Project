...
const UserDashboard = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#fdfaf3]">
      <div className="flex flex-col md:flex-row p-5 md:p-10 rounded-lg">
        <div className="w-full md:w-[600px] md:ml-20 bg-[#fffdf9] shadow-lg rounded-lg text-center border-blue-100-4 p-5">
          <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-6 mt-2">Your Dashboard</h2>
          <div className="flex flex-col items-center">
            <img
              src={user.profilePic || "https://via.placeholder.com/100"}
              alt="Profile"
              className="w-24 h-24 md:w-28 md:h-28 rounded-full border-4 border-gray-300 shadow-md mb-4"
            />
            <h3 className="text-lg md:text-xl font-semibold text-gray-700">{user.name || "Guest User"}</h3>
            <p className="text-gray-600">{user.email || "No email available"}</p>
            <p className="text-gray-600">Contact: {user.contact || "Not provided"}</p>
          </div>
          <div className="mt-6 flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4 md:justify-center">
            <button
        ...
        <div className="w-full md:w-auto mt-5 md:mt-0 md:ml-5 shadow-lg rounded-lg">
          <img src={petimg} alt="Pet" className="w-full md:w-auto rounded-lg" />
        </div>
      </div>
      ...
      <div className="min-h-screen py-10 px-5">
        <div className="max-w-6xl mx-auto mb-10">
          <h2 className="text-2xl md:text-3xl font-bold mb-4 text-green-600">🐶 Adopted Pets</h2>
        ...
        <div className="max-w-6xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold mb-4 text-pink-600">❤ Favorite Pets</h2>
          {favorites.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {favorites.map((pet) => (
                <div key={pet.id} className="bg-white shadow-md rounded-lg overflow-hidden">
